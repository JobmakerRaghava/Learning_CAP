...
async function run(param, options) {
    ...
    const { migrate,  cleanupMetatenants, syncTenantList: synchronizeTenantList, _hasMtEnv, getDbId } = require('../lib/migration/migration')
    ...
    async function cds_mtx_migrate(tenants, options) {
        ...
        const deploymentServiceConfig = cds.env.requires['cds.xt.DeploymentService'] || {}
        if (!deploymentServiceConfig.hdi?.create?.database_id) {
            const foundDbId = await getDbId()
            if (foundDbId) {
                cds.env.requires['cds.xt.DeploymentService'] = {
                    ...deploymentServiceConfig,
                    hdi: { ...deploymentServiceConfig.hdi,
                        create: {
                            ...deploymentServiceConfig.hdi?.create,
                            database_id: foundDbId
                        }
                    }
                }
            }
